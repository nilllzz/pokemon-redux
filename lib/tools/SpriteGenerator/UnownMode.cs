    ...
    class UnownMode
    {
        public static void Start()
        {
            ...
            for (int letterIndex = 0; letterIndex < letters.Length; letterIndex++)
            {
                ...
                {
                    var fFile = Path.Combine(inFolder, $"Spr_2{c}_{numStr}{letter}.png");
                    if (inFront == null && File.Exists(fFile))
                    {
                        inFront = new Bitmap(fFile);
                    }
                    var bFile = Path.Combine(inFolder, $"Spr_b_2{c}_{numStr}{letter}.png");
                    if (inBack == null && File.Exists(bFile))
                    {
                        inBack = new Bitmap(bFile);
                    }
                    var fsFile = Path.Combine(inFolder, $"Spr_2{c}_{numStr}{letter}_s.png");
                    if (inFrontS == null && File.Exists(fsFile))
                    {
                        inFrontS = new Bitmap(fsFile);
                    }
                    var bsFile = Path.Combine(inFolder, $"Spr_b_2{c}_{numStr}{letter}_s.png");
                ...
                var imgFile = Path.Combine(solutionFolder, "PTCG/Content/Textures/Pokemon/Main", number + "_" + letterIndex + ".png");
                ...
                //var dataFile = Path.Combine(solutionFolder, "PTCG/Content/Data/Pokemon", number + ".json");
